...
win = 0  # Сбито
hp = 10 # Жизни
TO_WIN = 10 # Очки для победы
TO_LOOSE = 10  # Очки для поражения
W_W = 700
W_H = 500
...
class Player(GameSprite):
    def update(self):
        ...
        if key_pressed[K_RIGHT] and self.rect.x + self.w + self.speed < W_W:
            self.rect.x += self.speed
        elif key_pressed[K_LEFT] and self.rect.x - self.speed > 0:
            self.rect.x -= self.speed
        super().update()
...
rocket = Player("rocket.png", 325, 410, 72, 90, 3)
...
while not game_over:
    ...
    for a in asteroids:
        if a.rect.y > W_H:
            a.reset()
    #TODO Надо бы добавить перезарядку оружия
    #! Этот код переработать!
    ...
    if is_gameplay:
        ...
        if ammo == 0:
            text("Перезарядка...", (255, 255, 255), (W_W // 2, W_H - 30))
